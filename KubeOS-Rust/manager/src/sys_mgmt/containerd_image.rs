...
impl<T: CommandExecutor> CtrImageHandler<T> {
    ...
    fn get_rootfs_archive(&self, req: &UpgradeRequest, permission: u32) -> Result<()> {
        let image_name = &req.container_image;
        let mount_path = &self.paths.mount_path.to_str().ok_or_else(|| {
            anyhow!(
                "Failed to get mount path: {}",
                self.paths.mount_path.display()
            )
        })?;
    ...
    fn check_and_unmount(&self, mount_path: &str) -> Result<()> {
        let ctr_snapshot_cmd = format!(
            "ctr -n={} snapshots ls | grep {} | awk '{{print $1}}'",
            DEFAULT_NAMESPACE, mount_path
        );
        let exist_snapshot = self
            .executor
            .run_command_with_output("bash", &["-c", &ctr_snapshot_cmd])?;
        if !exist_snapshot.is_empty() {
            self.executor.run_command(
                "ctr",
                &["-n", DEFAULT_NAMESPACE, "images", "unmount", mount_path],
            )?;
            self.executor.run_command(
                "ctr",
                &["-n", DEFAULT_NAMESPACE, "snapshots", "remove", mount_path],
            )?;
        }
        Ok(())
    }
...
mod tests {
    ...
    fn test_get_image() {
        ...
        mock_executor
            ...
            .withf(|cmd, args| {
                cmd == "crictl"
                    && args.contains(&"pull")
                    && args.contains(&"docker.io/library/busybox:latest")
            })
            .times(1)
            .returning(|_, _| Ok(()));
        ...
        mock_executor
            ...
            .withf(|cmd, args| {
                cmd == "crictl"
                    && args.contains(&"inspecti")
                    && args.contains(&"{{.status.repoDigests}}")
            })
            .times(1)
    ...
    fn test_copy_file() {
        ...
        let actual_content =
            fs::read_to_string(&dst_path).expect("Failed to read destination file.");
    ...
    fn test_check_and_unmount() {
        ...
        let result = CtrImageHandler::new(PreparePath::default(), mock_executor)
            .check_and_unmount("test_mount_path");
