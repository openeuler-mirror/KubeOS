...
* 升级失败（或执行过程中断）时，为便于定位问题，残留的临时文件和挂载不会被立即清理；下次执行升级时会先对残留的升级目录和挂载进行清理，再开始新的升级流程，无需人工干预。残留文件本身不影响系统运行。
